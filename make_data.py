...
class DataMake:
    ...
    def make(self):
        ...
        ### clade information
        clade_info = pd.read_csv(self.clade_csv, index_col=0)
        ...
        position_data["clade"] = position_data["clade"].astype(str)
    ...
    def make_dist(self, data_for_dist):
        ...
        for i in range(len(mistake_index[0])):
            ...
            if first.start < second.start:
                if second.start - first.end < 0:
                    distance.iloc[mistake_index[0][i], mistake_index[1][i]] = -1
                else:
                    distance.iloc[mistake_index[0][i], mistake_index[1][i]] = second.start - first.end
            else:
                if first.start - second.end < 0:
                    distance.iloc[mistake_index[0][i], mistake_index[1][i]] = -1
                else:
    ...
    def make_z_data(self, dist, type, position_data):
        ...
        ### matrix based on MADA HMM score
            ...
            MADA = copy.copy(position_data["HMM_score"])
            MADA[position_data["Seq-F"] > 30] = 0
            MADA = MADA[::-1]
            MADA = MADA.fillna(0)
